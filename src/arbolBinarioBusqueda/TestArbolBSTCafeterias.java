...
public class TestArbolBSTCafeterias {
    private static ArbolBST<Cafeteria> arbol = null;  // Instancia del árbol de cafeterias
    private static Cafeteria cafeteria;
    private static List<Cafeteria> listaCafeterias = new ArrayList<>(); // Lista para guardar los hospitales
        ...
        do {
            ...
            switch (opcion) {
                ...
                case 12:
                    ...
                    break;
                case 13:
                    iniciarInterfazGrafica(listaCafeterias);
                    break;
                case 14:
    ...
    // Métodos estáticos
    public static void iniciarInterfazGrafica(List<Cafeteria> cafeterias) {
        ArbolBST<Cafeteria> arbolGrafico = new ArbolBST<>();

        // Insertar cada elemento de la lista en el árbol gráfico
        for (Cafeteria cafeteria : cafeterias) {
            arbolGrafico.insertar(cafeteria);
        }

        // Crear la interfaz gráfica y mostrarla
        GuiBST<Cafeteria> gui = new GuiBST<>(arbolGrafico);
        gui.setSize(600, 600);
    ...
    public static void agregarElemento() {
        if (arbol != null){
            System.out.print("Ingrese el nombre de la cafeteria: ");
            String nombre = sc.nextLine();
            System.out.print("Ingrese la ubicación de la cafeteria: ");
            ...
            while (!numExtValido) {
                try {
                    System.out.print("Ingrese el numero exterior de la cafeteria: ");
                    numExt = sc.nextInt();  // Intentar convertir la entrada a entero
            ...
            cafeteria = new Cafeteria(nombre,ubicacion,numExt);
            arbol.insertar(cafeteria);
            listaCafeterias.add(cafeteria);  // Añadir elemento a la lista
    ...
    public static void eliminarElemento() {
        if (arbol != null){
            System.out.print("Ingrese el nombre de la cafeteria a eliminar: ");
            String nombre = sc.nextLine();
    ...
    public static void buscarElemento() {
        if (arbol != null){
            System.out.print("Ingrese el nombre de la cafeteria a buscar: ");
            String nombre = sc.nextLine();

            cafeteria = arbol.buscar(nombre);
            if (cafeteria != null) {
                System.out.println("Elemento encontrado: " + cafeteria);
            } else {
